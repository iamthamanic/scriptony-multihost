...
function buildState(redirectUri: string, provider: string, userId: string): string {
...
function parseState(state: string): { redirect_uri: string; provider: string; user_id: string } | null {
...
app.get("/authorize", (c) => {
  ...
  if (!provider || !redirectUri) {
    return c.json(
      { success: false, error: { code: "BAD_REQUEST", message: "Missing provider or redirect_uri" } },
      400,
    );
  }
  ...
  if (!cfg) {
    return c.json(
      { success: false, error: { code: "BAD_REQUEST", message: `Unknown provider: ${provider}` } },
      400,
    );
  }
  ...
  if (!clientId) {
    return c.json(
      { success: false, error: { code: "NOT_CONFIGURED", message: "OAuth not configured for this provider" } },
      503,
    );
  }
  ...
  if (!callbackUrl) {
    return c.json(
      { success: false, error: { code: "NOT_CONFIGURED", message: "OAuth callback URL is not set" } },
      503,
    );
  }
...
app.get("/callback", async (c) => {
  ...
  if (!code || !state) {
    return c.json(
      { success: false, error: { code: "BAD_REQUEST", message: "Missing code or state" } },
      400,
    );
  }
  ...
  if (!statePayload?.redirect_uri) {
    return c.json(
      { success: false, error: { code: "BAD_REQUEST", message: "Invalid state" } },
      400,
    );
  }
  ...
  if (!isRedirectUriAllowed(statePayload.redirect_uri)) {
    return c.json(
      { success: false, error: { code: "BAD_REQUEST", message: "Invalid redirect_uri" } },
      400,
    );
  }
  ...
  try {
    ...
    switch (statePayload.provider) {
      ...
      default:
        return c.json(
          { success: false, error: { code: "NOT_IMPLEMENTED", message: `OAuth for ${statePayload.provider} not implemented yet` } },
          501,
        );
    }
    ...
    return c.json(
      { success: false, error: { code: "TOKEN_EXCHANGE_FAILED", message: "Token exchange failed" } },
      500,
    );
  }
});
