...
import { LOCAL_DEV_BEARER } from "./local-dev-auth";

/** Only when explicitly enabled on the function (never in production Appwrite by default). */
function isLocalDevBearerAllowed(): boolean {
  const v = process.env.SCRIPTONY_ALLOW_LOCAL_DEV_BEARER?.trim().toLowerCase();
...
export async function resolveAuthenticatedUser(
  ...
  // Local desktop mode: the SPA sends a fixed marker token so the
  // function does not need a real Appwrite JWT. Return a synthetic
  // user that satisfies downstream checks (bootstrap, RLS, etc.).
  const token = getBearerToken(authHeader);
  if (token === LOCAL_DEV_BEARER && isLocalDevBearerAllowed()) {
    return { id: "local-user", displayName: "Local User", defaultRole: "user" };
  }
