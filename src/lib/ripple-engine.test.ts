...
import { calculateRipple, checkForConflict } from "./ripple-engine";
import type { AudioClip } from "./types";

function makeClip(id: string, sceneId: string, startSec: number, endSec: number, overrides: Partial<AudioClip> = {}): AudioClip {
	return { id, trackId: `track-${id}`, sceneId, projectId: "proj-1", startSec, endSec, laneIndex: 0, orderIndex: 0, trackType: "dialog", content: "Text", createdAt: "2024-01-01", updatedAt: "2024-01-01", ...overrides };
}
function makeScene(id: string, sequenceId: string, orderIndex: number, startSec: number, endSec: number) {
	return { id, sequenceId, startSec, endSec, durationSec: endSec - startSec, orderIndex };
}
function makeSequence(id: string, actId: string, orderIndex: number, startSec: number, endSec: number) {
	return { id, actId, startSec, endSec, durationSec: endSec - startSec, orderIndex };
}
function makeAct(id: string, orderIndex: number, startSec: number, endSec: number) {
...
describe("calculateRipple", () => {
	it("delta = 0 → keine Änderungen", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 10, allClips: [clip], allScenes: [scene], allSequences: [seq], allActs: [act] });
		expect(result.stats.deltaSec).toBe(0);
		expect(result.stats.affectedClips).toBe(0);
	});

	it("throwt wenn Clip nicht gefunden", () => {
		expect(() => calculateRipple({ changedClipId: "missing", newEndSec: 20, allClips: [], allScenes: [], allSequences: [], allActs: [] })).toThrow("Clip not found: missing");
	});

	it("throwt wenn Scene nicht gefunden", () => {
		const clip = makeClip("c1", "missing-scene", 0, 10);
		expect(() => calculateRipple({ changedClipId: "c1", newEndSec: 20, allClips: [clip], allScenes: [], allSequences: [], allActs: [] })).toThrow("Scene not found for clip: missing-scene");
	});
	...
	it("Clip verlängert → Scene.endSec erhöht", () => {
		...
		const seq = makeSequence("sq1", "a1", 0, 0, 10);
		const act = makeAct("a1", 0, 0, 10);
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 20, allClips: [clip], allScenes: [scene], allSequences: [seq], allActs: [act] });
	...
	it("Clip verlängert → nachfolgende Scene in Sequence verschoben", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 25, allClips: [c1, c2], allScenes: [s1, s2], allSequences: [sq1], allActs: [a1] });
	...
	it("Scene verlängert → nachfolgende Sequence in Act verschoben", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 30, allClips: [c1, c2], allScenes: [s1, s2, s3], allSequences: [sq1, sq2], allActs: [a1] });
		expect(result.updatedScenes.find(s => s.id === "s2")!.startSec).toBe(30);
		expect(result.updatedSequences.find(sq => sq.id === "sq2")!.startSec).toBe(40);
	...
	it("Sequence verlängert → nachfolgender Act verschoben", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 30, allClips: [c1, c2], allScenes: [s1, s4], allSequences: [sq1, sq3], allActs: [a1, a2] });
	...
	it("Clip kürzer → nachfolgende Scenes rutschen nach links", () => {
		...
		const a1 = makeAct("a1", 0, 0, 30);
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 10, allClips: [c1, c2], allScenes: [s1, s2], allSequences: [sq1], allActs: [a1] });
	...
	it("negative Delta schützt gegen startSec < 0", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 2, allClips: [c1, c2], allScenes: [s1, s2], allSequences: [sq1], allActs: [a1] });
		expect(result.updatedClips.find(c => c.id === "c2")!.startSec).toBeGreaterThanOrEqual(0);
	});
	...
	it("crossScene = true → Clip wird NICHT verschoben", () => {
		...
		const sq1 = makeSequence("sq1", "a1", 0, 0, 20);
		const a1 = makeAct("a1", 0, 0, 20);
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 20, allClips: [c1, c2], allScenes: [s1, s2], allSequences: [sq1], allActs: [a1] });
		expect(result.updatedClips.find(c => c.id === "c2")!.startSec).toBe(10);
	});
	...
	it("Scene-Dauer = max(clip.endSec), nicht Summe", () => {
		...
		const sq1 = makeSequence("sq1", "a1", 0, 0, 30);
		const a1 = makeAct("a1", 0, 0, 30);
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 5, allClips: [c1, c2], allScenes: [s1], allSequences: [sq1], allActs: [a1] });
	...
	it("3 Szenen in Sequence → nur nachfolgende verschoben", () => {
		...
		const sq1 = makeSequence("sq1", "a1", 0, 0, 30);
		const a1 = makeAct("a1", 0, 0, 30);
		const result = calculateRipple({ changedClipId: "c2", newEndSec: 25, allClips: [c1, c2, c3], allScenes: [s1, s2, s3], allSequences: [sq1], allActs: [a1] });
	...
	it("3 Sequences in Act → nur nachfolgende verschoben", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 20, allClips: [c1, c2], allScenes: [s1, s4], allSequences: [sq1, sq2, sq3], allActs: [a1] });
		expect(result.updatedSequences.find(sq => sq.id === "sq1")!.endSec).toBe(20);
		expect(result.updatedSequences.find(sq => sq.id === "sq2")!.startSec).toBe(20);
		expect(result.updatedSequences.find(sq => sq.id === "sq3")!.startSec).toBe(40);
		expect(result.updatedClips.find(c => c.id === "c2")!.startSec).toBe(40);
	});
	...
	it("3 Acts → nur nachfolgende verschoben", () => {
		...
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 25, allClips: [c1, c2], allScenes: [s1, s2], allSequences: [sq1, sq4], allActs: [a1, a2, a3] });
	...
	it("komplette Kaskade: Clip → Scene → Sequence → Act", () => {
		const clips = [makeClip("c1", "s1", 0, 10), makeClip("c2", "s2", 20, 30), makeClip("c3", "s3", 50, 60)];
		const scenes = [makeScene("s1", "sq1", 0, 0, 10), makeScene("s2", "sq2", 0, 20, 30), makeScene("s3", "sq3", 0, 50, 60)];
		const seqs = [makeSequence("sq1", "a1", 0, 0, 10), makeSequence("sq2", "a1", 1, 20, 30), makeSequence("sq3", "a2", 0, 50, 60)];
		const acts = [makeAct("a1", 0, 0, 30), makeAct("a2", 1, 50, 60)];
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 30, allClips: clips, allScenes: scenes, allSequences: seqs, allActs: acts });
	...
	it("stats korrekt bei einfacher Änderung", () => {
		const clip = makeClip("c1", "s1", 0, 10);
		const scene = makeScene("s1", "sq1", 0, 0, 10);
		const seq = makeSequence("sq1", "a1", 0, 0, 10);
		const act = makeAct("a1", 0, 0, 10);
		const result = calculateRipple({ changedClipId: "c1", newEndSec: 15, allClips: [clip], allScenes: [scene], allSequences: [seq], allActs: [act] });
		expect(result.stats.affectedClips).toBe(1);
		expect(result.stats.affectedScenes).toBe(1);
		expect(result.stats.affectedSequences).toBe(1);
		expect(result.stats.affectedActs).toBe(1);
...
describe("checkForConflict", () => {
	it("false wenn updatedAt gleich", () => {
		...
		expect(checkForConflict({ clipId: "c1", lastKnownUpdatedAt: "2024-01-01", allClips: [clip] })).toBe(false);
	});
	it("true wenn updatedAt unterschiedlich", () => {
		const clip = makeClip("c1", "s1", 0, 10, { updatedAt: "2024-01-02" });
		expect(checkForConflict({ clipId: "c1", lastKnownUpdatedAt: "2024-01-01", allClips: [clip] })).toBe(true);
	});
	it("false wenn Clip gelöscht", () => {
		expect(checkForConflict({ clipId: "missing", lastKnownUpdatedAt: "2024-01-01", allClips: [] })).toBe(false);
	});
});
