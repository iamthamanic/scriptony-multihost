...
export const LANE_UI = {
  ...
  heightDialogCompact: 210,
  heightDialogExpanded: 320,
  /** Dialog lane with zero text blocks/clips — no inline card to fit, so it
   * matches the standard compact lane height instead of wasting space. */
  heightDialogEmpty: 152,
 ...
 * `hasContent` shrinks empty dialog lanes (no text blocks/clips yet) to the
 * standard compact height instead of reserving the full inline-editor
 * height. Defaults to `true` for backward compatibility with callers that
 * don't track lane content.
 */
export function resolveLaneHeightPx(
  ...
  if (isDialogCharacterLane) {
    if (expanded) return LANE_UI.heightDialogExpanded;
    return hasContent ? LANE_UI.heightDialogCompact : LANE_UI.heightDialogEmpty;
  }
