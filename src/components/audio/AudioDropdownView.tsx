/**
 * AudioDropdownView — Hörbuch/Hörspiel Dropdown-Ansicht.
...
import { SceneImageUploadField } from "../timeline/SceneImageUploadField";
...
interface AudioDropdownViewProps {
  projectId: string;
  projectType?: string;
}
...
export function AudioDropdownView({
  projectId,
  projectType,
}: AudioDropdownViewProps) {
  const clipSystemEnabled = isAudioClipSystemEnabled(projectType);
  ...
  const { uploadForScene, uploadingSceneId } = useSceneImageUpload(projectId);
  ...
  const handleSceneImageUpload = (sceneId: string, file: File) => {
    void uploadForScene(sceneId, file);
  };
      ...
      <div className="flex items-center gap-2 p-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem
              onClick={() =>
                void pickImageFile().then((file) => file && onUploadImage(file))
              }
            >
    ...
    <div className="flex items-center gap-2 px-2 py-1.5 rounded-md bg-white/80 dark:bg-black/20 border border-pink-100 dark:border-pink-800/50 hover:border-pink-300 dark:hover:border-pink-600 transition-colors group">
      ...
      <div className="flex-1 min-w-0">
        ...
        {(track.audioDuration ?? 0) > 0 &&
          !isAudioClipSystemEnabled("audio") && (
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
        {isTTS && isAudioClipSystemEnabled("audio") && (
          <TtsGenerateButton
            trackId={track.id}
...
export default AudioDropdownView;
