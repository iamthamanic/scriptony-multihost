...
import { DropdownViewMobile } from "./DropdownViewMobile";
...
import { useOptimizedDropdownView } from "../../hooks/useOptimizedDropdownView";
...
export type { TimelineData } from "../../lib/timeline-data";
...
export function DropdownView({
  ...
  const optimized = useOptimizedDropdownView({
    acts,
    sequences,
    scenes,
    shots,
  ...
  const ensureSceneShotsLoaded = useCallback(
    async (sceneId: string) => {
      ...
      try {
        ...
        console.error(
          "[DropdownView] Failed lazy loading shots for scene:",
          sceneId,
          error,
        );
      } finally {
  ...
  useEffect(() => {
    ...
    console.time(`⏱️ [PERF] DropdownView Full Load: ${projectId}`);
  ...
  useEffect(() => {
    if (externalCharacters) {
      console.log(
        "[DropdownView] External characters updated:",
  ...
  const loadTimelineData = async () => {
    ...
    try {
      ...
      if (cached.data) {
        console.log(
          `[DropdownView] 💾 Loading from cache (${cached.isStale ? "stale" : "fresh"})`,
        );
        ...
        console.log(
          "[DropdownView] 🔄 Revalidating stale cache in background...",
        );
      }
      ...
      if (!externalCharacters) {
        try {
          ...
          console.error(
            "[DropdownView] Character fallback load failed:",
            characterError,
          );
          loadedCharacters = [];
        }
      ...
      console.log("[DropdownView] ✅ Parallel load complete:", {
      ...
      if (!externalCharacters) {
        console.log(
          "[DropdownView] Using characters from parallel load:",
          loadedCharacters.length,
        );
        setCharacters(loadedCharacters);
      } else {
        console.log(
          "[DropdownView] Using characters from parent:",
          externalCharacters.length,
        );
      }
  ...
  const handleInitializeNarrativeStructure = async () => {
    ...
    try {
      ...
      console.error(
        "[DropdownView] initializeTimelineStructureFromNarrative:",
        e,
      );
      toast.error(
        e instanceof Error
  ...
  const handleDeleteAct = async (actId: string) => {
    ...
    if (!actToDelete) {
      console.error("[DropdownView] Act not found for deletion:", actId);
      toast.error("Act nicht gefunden");
      return;
    }

    console.log("[DropdownView] 🗑️ Deleting act:", {
      id: actId,
    ...
    if (
      ...
      console.log("[DropdownView] Deletion cancelled by user");
      return;
    }
    ...
    console.log("[DropdownView] Deleting act with:", {
    ...
    try {
      ...
      console.log("[DropdownView] Calling API to delete act:", actId);
      await TimelineAPI.deleteAct(actId, token);
      console.log("[DropdownView] ✅ Act deleted successfully");
  ...
  useEffect(() => {
    if (
      ...
      console.log("🚀 [DropdownView] Performance Stats:", {
        totalItems: {
          acts: acts.length,
