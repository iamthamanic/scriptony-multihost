...
#[tauri::command]
pub fn start_voicebox_app(app_name: Option<String>) -> Result<String, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        let status = Command::new("open")
            ...
            .arg(&name)
            .status()
            .map_err(|e| format!("Voicebox konnte nicht gestartet werden ({name}): {e}"))?;

        if !status.success() {
            return Err(format!(
                "Voicebox-App \"{name}\" konnte nicht geöffnet werden. Bitte manuell starten."
            ));
        }
